...
class MetadataContentTest {
  ...
  fun `TimeBasedActivation should handle different time scenarios`() {
    ...
    fun isWithinActivationPeriod(time: Instant, activation: MetadataContent.TimeBasedActivation): Boolean {
      return time > activation.startTime && time <= activation.endTime
    }
  ...
  fun `GeographicTargeting should handle country and region lists`() {
    val targeting = MetadataContent.GeographicTargeting(
  ...
  fun `DeviceTargeting should handle platform and device type lists`() {
    val targeting = MetadataContent.DeviceTargeting(
  ...
  fun `CustomRules should handle key-value pairs`() {
    val rules = MetadataContent.CustomRules(
      rules = mapOf(
